...
```mermaid
flowchart TD
    Q[Вопрос пользователя] --> ORCH[Agentic orchestrator<br/>self-RAG loop]
...
- **Postgres + pgvector** — один стор для метаданных, полнотекста (FTS с русской
  ...
  **Статус: спроектировано, не подключено.** В репозитории есть
  `src/praxis/index/schema.sql`; ретривера поверх него нет, и `psycopg` не
  импортируется ни в одном модуле. Сегодня плотный поиск обслуживает
  `src/praxis/retrieve/dense.py` — векторы в памяти с кэшем на диск и запасным
  путём на чистой стандартной библиотеке.
...
Два оркестратора с одним контрактом `.answer(question) -> Answer`:
- **SelfRAG** — детерминированный прозрачный луп без внешних зависимостей (дефолт):
  `план → поиск → оценка достаточности → доп-поиск/переформулировка → генерация`.
  Останавливается, когда набрано достаточно оснований, либо честно сообщает, что
  ответа в законе нет.
- **GliaLegalAgent** — LLM-режим (при наличии ключа) на библиотеке **glia**
  (DrobyshevDev): поиск норм оформлен как glia-инструмент `search_law`, цикл ведёт
  `glia.Agent`, трейс рассуждения берётся из glia-trajectory (glass-box). Тестируется
  оффлайн через scripted `glia.providers.EchoLLM`.
...
- **Регрессии:** прогон на каждом релизе; drift-мониторинг — в планах (кандидат — mlango).
...
3. **Один Postgres на старте** — минимум инфраструктуры, быстрый путь к рабочему v1.
   Ставка сделана, но **не сыграна**: индекс пока в памяти (см. раздел 2).
4. **Догфудинг экосистемы DrobyshevDev** — **glia** подключена (LLM-режим: агент,
   инструмент поиска, trajectory). **mlango** подключён (трекаемый golden-eval через его
   подсистему evals — `integrations/mlango_eval`, `manage.py evaluate`).
